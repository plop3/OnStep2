...
#define MinutesPastMeridianE      10 // for goto's, how far past the meridian to allow before we do a flip (if on the East side of the pier) - a half hour of RA is the default = 30.  Ignored on Alt/Azm mounts.
#define MinutesPastMeridianW      10 // as above, if on the West side of the pier.  If left alone, the mount will stop tracking when it hits the this limit.  Ignored on Alt/Azm mounts.
...
#define AXIS3_REVERSE_ON            // reverse the direction of Axis3 rotator movement.
...
#define AXIS4_DISABLE HIGH            // Pin 24 (Aux4.)  Default _OFF, use HIGH for common stepper drivers
#define AXIS4_AUTO_POWER_DOWN_ON    // use _ON if you want to power down the motor at stand-still.  Default _OFF.  Ignored in DC motor mode.
...
#define MaxAxis5                45.0 // maximum allowed position in millimeters, default =  25.0
...
#define AXIS5_DISABLE HIGH            // Pin 30 (Aux5.) Default _OFF, use HIGH for common stepper drivers
#define AXIS5_AUTO_POWER_DOWN_ON    // use _ON if you want to power down the motor at stand-still.  Default _OFF.  Ignored in DC motor mode.
